...
split<-lessC %>% 
  ...
  select(trt, plot, depth, rootC, carbon)#%>%
  #group_by(trt, depth)%>%
  #summarise_each(funs(mean))%>%
  #gather(key = variable, value = value, root:carbon)

write.csv(split, file="Equivalent root and C for 5 cm, each plot.csv", row.names=FALSE, quote=FALSE)



ggplot(split, aes(y=root, x=-depth))+
...
ggplot()+
  geom_line(aes(x=-split$depth, y=split$root), color="green")+
  geom_line(aes(x=-split$depth, y=split$carbon), color="brown")+
  coord_flip()+
  facet_wrap(~trt)
...
g<-ggplot(meltprop, aes(x=factor(depth), y = value, group=variable)) + 
    geom_point(aes(colour=variable), size=4)+#(stat = "identity", position = "dodge") +
